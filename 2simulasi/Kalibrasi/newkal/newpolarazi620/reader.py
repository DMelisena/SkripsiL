...
plt.plot(datax, datay,label='Data Mentah 10 $\it{batch}$')
tal = sp.tallies[1]
datay = tal.mean
datay=datay/y_max*100
n = len(datay)
...
plt.plot(x_smooth, y_smooth,label='$\it{Trendline}$ 10 $\it{batch}$')
#plt.title(f'x_max = {x_max} cm \n xsmooth_max = {x_maxs} cm')
plt.title(f'Hasil Kalibrasi 1e9 partikel dengan 10 batch')
plt.legend()
plt.savefig('pdd.png')
plt.show()
"""
